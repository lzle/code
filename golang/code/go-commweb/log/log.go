...
func (l *Logger) Init() {
	...
	expression := "0 */1 * * *"
	cronJob := cron.New()
	err := cronJob.AddFunc(expression,l.rotation)
	if err != nil {
...
func (l *Logger) logFile() string {
	return l.logDir() + "/log.txt"
}


func (l *Logger) logDir() string {
	dir := config.ConfigParam.LogPathConfig.Path
	exists := common.PathExists(dir)
...
func (l *Logger) rotation(){
	...
	destPath := l.logFile() + "@" + common.ConvertTimeToLogTimeSuffix(common.LastHour())
	//rename the log file.
